...
//   uint packed_hemi = ( MUST_BE_SET + ( uint( hemi * 255.0 ) << 13 ) + ( ( packed_mtl & uint( 31 ) ) << 21 ) );
   ...
   if( ( packed_hemi & USABLE_BIT_13 ) == uint(0) )
      packed_hemi |= USABLE_BIT_14;

   if( ( packed_mtl & uint( 16 ) ) != uint(0) )
...
//   return float( ( asuint( mtl_hemi ) >> 13 ) & uint(255) ) * (1.0/255.0);
	return float( ( asuint( mtl_hemi ) >> uint(13) ) & uint(255) ) * (1.0/254.8);
}

float gbuf_unpack_mtl( float mtl_hemi )
{
   uint packed_mtl       = asuint( mtl_hemi );
   uint packed_hemi  = ( ( packed_mtl >> uint(21) ) & uint(15) ) + ( ( packed_mtl & USABLE_BIT_15 ) == uint(0) ? uint(0) : uint(16) );
...
{
   return gbuffer_load_data( tc, pos2d, uint(0) );
}
...
gbuffer_data gbuffer_load_data_offset( float2 tc, float2 OffsetTC, float4 pos2d )
{
	...
	return gbuffer_load_data( OffsetTC, pos2d + delta, uint(0) );
}
